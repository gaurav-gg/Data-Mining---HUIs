...
public class AlgoFHN {
	...
	public TreeMap<Long,StringBuilder> topk = new TreeMap<Long,StringBuilder>();
	public int tk ;
	public long minUtility = 0;
	...
	* the idea is to always reuse the same buffer to reduce memory usage. */
	 ...
	 */
	public void runAlgorithm(String input, String output, int tk) throws IOException {
		// reset maximum
	 ...
	 * @throws IOException
	 */
	//=================== adding forward extension, backward Extension and closure jumping methods.===========================================
	private void fhn(int [] prefix, 
			int prefixLength, UtilityListFHN pUL, List<UtilityListFHN> ULs, long minUtility)
			throws IOException {
		boolean hasBackwardExtension, closureJumping, hasForwardExtension;
		// For each extension X of prefix P
		for(int i=0; i< ULs.size(); i++){
			UtilityListFHN X = ULs.get(i);
			ArrayList<Integer> xTids = new ArrayList<Integer>();
			for(ElementFHN eX : X.elements){
				xTids.add(eX.tid);
			}
			hasBackwardExtension = false;
			hasForwardExtension = false;
			//closureJumping = true;
			// check backward Extension
			for(int j=i-1;j>=0;j--){
				UtilityListFHN A = ULs.get(j);
				HashSet<Integer> aTids = new HashSet<Integer>();
				for(ElementFHN eA : A.elements){
					aTids.add(eA.tid);
				}
				int k = 0;
				for(;k<xTids.size();k++){
					if(!aTids.contains(xTids.get(k))){
						break;
					}
				}
				if(k==xTids.size()){
					hasBackwardExtension = true;
					break;
				}
			}
			// check forward extension and closure jumping
			for(int j=i+1;j<ULs.size();j++){
				...
				/*else{
					closureJumping = false;
				}*/
			}
			// If pX is a high utility itemset.
			// we save the itemset:  pX 
			if((X.sumIutils + X.sumINutils >= minUtility)&&!hasBackwardExtension){
				// save to file
				/*if(closureJumping){
					long cjUtility = 0;
					ArrayList<Integer> cjList = new ArrayList<Integer>(); 
					for(int j=i+1;j<ULs.size();j++){
						UtilityListFHN B = ULs.get(j);
						cjUtility += (B.sumIutils+B.sumINutils);
						cjList.add(B.item);
					}
					writeOut(prefix, prefixLength, cjList, cjUtility);
				}*/
			...
			if((X.sumIutils + X.sumRutils >= minUtility)/*&&!closureJumping*/){
	 ...
	 */
	private UtilityListFHN construct(UtilityListFHN P, UtilityListFHN px, UtilityListFHN py, long minUtility) {
	...
	//write to a file===============topk==================
	private void writeOut() throws IOException {
		for(Map.Entry<Long,StringBuilder> entry : topk.entrySet()){
	 ...
	 */
	 //====================New Write Out Method for Closure Jumping==================================
	 private void writeOut(int[] prefix, int prefixLength, ArrayList<Integer> items,long utility) throws IOException {
		...
		//==================================topk=======================================================
		if(topk.size()<tk){
			topk.put(utility,buffer);
		}
		else{
			long minKey = topk.firstKey();
			if(minKey<utility){
				topk.remove(minKey);
				topk.put(utility,buffer);
			}
	...
	public void printStats() throws IOException {
		...
		System.out.println(" High-utility itemsets count : " + Math.min(huiCount,tk)); 
